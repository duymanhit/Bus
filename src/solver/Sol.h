...
inline Sol::Sol()
{
    //cout << "manh ";
    total_cost = 0;
    //initArray(this->order_node, n_node + 1, 0);
    //initArray(this->giant_tour, n_node * 2, 0);
...
void Sol::split() {
    ///<summary>
    /// f_split[i][k] is result of splitting to i with k nagative locations
    ///f_split[i][j] =
    ///</summary>
    ...
    for (i = 0; i < n_node; i++) {
        for (k = 0; k <= max_negative_students; k++) if (f_split[i][k] != oo) {
                demand = 0;
                time = 0;
                start = 0;
    ...
    for (k = 0; k <= max_negative_students; k++) if (f_split[n_node][k] < f_split[n_node][current_n_negative]) {
            current_n_negative = k;
        }
    ...
    giant_tour[++n_giant_tour] = 0;
    convertGiantToRoutes();
    //check();
    //update();
}
void Sol::convertGiantToRoutes() {
    // remove double depots
    int n = 0;
    for (int i = 1; i <= n_giant_tour; i++) if (giant_tour[i - 1] != 0 || giant_tour[i] != 0)
    ...
    for (int i = 1; i <= n_giant_tour; i++) {
        if (giant_tour[i] == 0) {
            ...
            //fixed_cost_of_route[n_route] = fixed_cost_for_demand[demand_of_route[n_route]];
            //travel_cost_of_route[n_route] = time_giant[i - 1] * travel_cost_for_demand[demand_of_route[n_route]];
...
int Sol::check() {
    ...
    for (int i = 1; i <= n_giant_tour; i++) {
        if (giant_tour[i] == 0) {
            ...
            //if (fixed_cost_of_route[route] != fixed_cost_for_demand[demand]) {
            //	cout << "Error: fixed cost of one route" << endl;
            //}
            //if (travel_cost_of_route[route] != time * travel_cost_for_demand[demand]) {
            //	cout << "Error: travelling cost of one route" << endl;
            //}
            check_cost += fixed_cost_for_demand[demand] + time * travel_cost_for_demand[demand];
            // number of node
            if (node > max_nodes_per_route) {
                cout << "Error: max node per route" << endl;
                return 1;
            }
            if (time > max_time_per_route) {
                cout << "Error: max time per route" << endl;
                return 2;
            }
            // new route
            time = 0;
...
int Sol::getNegativeStudents(const int& start, const int& end, const int& time) {
    ...
    for (int i = start; i <= end; i++) {
        ...
        result += (checkNegative(giant_tour[i], time_giant[i] + time) - is_negative_node[giant_tour[i]]) * nodes[giant_tour[i]].demand;
    }
    return result;
}
...
inline int Sol::getNegativeStudentForm(const int& start, int time)
{
    ...
    for (int i = start; true; i++) {
        ...
        result += (checkNegative(cache_route[i], time) - is_negative_node[cache_route[i]]) * nodes[cache_route[i]].demand;
...
inline int Sol::getDiffTimeSwap(int& start_1, int& end_1, int& start_2, int& end_2)
{
    int current_time;
    if (start_1 <= end_1) {
        current_time = travel_time[giant_tour[start_1]][giant_tour[start_1 - 1]] + travel_time[giant_tour[end_1 + 1]][giant_tour[end_1]]
    ...
    if (start_2 <= end_2) {
        return travel_time[giant_tour[start_2]][giant_tour[start_1 - 1]] + travel_time[giant_tour[end_1 + 1]][giant_tour[end_2]]
               + getTimeSubTour(start_2, end_2)
               - current_time;
    }
    //out(travel_time[giant_tour[end_1 + 1]][giant_tour[start_1 - 1]], current_time);
...
//		return
//	}
//	return getDiffDemandSwap(start_1, end_1, start_2, end_2);
//}
...
void Sol::localSearch() {
    int max_k = max_nodes_per_route / 2 - 1;
            ...
            for (int end_1 = start_1 - 1; end_1 < min(start_1 + max_k, n_giant_tour); end_1++)
                for (int start_2 = start_1 + 1; start_2 < n_giant_tour; start_2++)
                    for (int end_2 = start_2 - 1; end_2 < min(start_2 + max_k, n_giant_tour); end_2++) {
                        ...
                        if ((route_1 != route_of_index[start_1] || giant_tour[start_1] == 0) && start_1 <= end_1) continue;
                        if ((route_2 != route_of_index[start_2] || giant_tour[start_2] == 0) && start_2 <= end_2) continue;
                        if (route_1 != route_2) {
                            // number node
                            if (end_route[route_1] - start_route[route_1] - (end_1 - start_1 + 1) + (end_2 - start_2 + 1) > max_nodes_per_route)
                                continue;
                            if (end_route[route_2] - start_route[route_2] + (end_1 - start_1 + 1) - (end_2 - start_2 + 1) > max_nodes_per_route)
                                continue;
                            //
                            ...
                            if (new_time_1 > max_time_per_route || new_time_2 > max_time_per_route)
                                continue;
                            if (cost_of_route[route_1] + cost_of_route[route_2] <= getCostRoute(new_demand_1, new_time_1) + getCostRoute(new_demand_2, new_time_2))
                                continue;
                            ...
                            // negative
                            //cout << checkTimeNode(giant_tour[end_2 + 1], time_giant[end_2 + 1] + diff_time_2) << "***" << endl;
                            negative_1 = getNegativeStudents(end_1 + 1, n_giant_tour, diff_time_1)
                                         + getNegativeStudentsWithStartTime(start_2, end_2, time_giant[start_1 - 1] + travel_time[giant_tour[start_2]][giant_tour[start_1 - 1]]);
                            negative_2 = getNegativeStudents(end_2 + 1, n_giant_tour, diff_time_2)
                                         + getNegativeStudentsWithStartTime(start_1, end_1, time_giant[start_2 - 1] + travel_time[giant_tour[start_1]][giant_tour[start_2 - 1]]);
                            ...
                            convertGiantToRoutes();
                            //if (check() != 0 || new_cost != total_cost) {
                            ...
                            swapSubArray(cache_route + start_route[route_1], end_route[route_1] - start_route[route_1] + 1,
...
inline void Sol::greedy()
{
    ...
    for (int t = 1; t <= n_node; t++) {
        ...
        for (int node = 1; node <= n_node; node++) if (!is_served[node]) {
...
inline void Sol::greedyFormCurrentSol()
{
    ...
    for (int i = 1; i <= n_node / 2; i++) is_served[order_node[i]] = false;
    int n = 0;
    for (int i = 1; i <= n_giant_tour; i++) if (giant_tour[i] == 0 || is_served[giant_tour[i]]) {
            giant_tour[++n] = giant_tour[i];
        }
    n_giant_tour = n;
    convertGiantToRoutes();
...
inline II Sol::getCostInsert(const int& node)
{
    int res_cost = oo, res_index = 0;
    for (int index = 0; index < n_giant_tour; index++) {
        // number node of this route
        int route = route_of_index[index];
        if (end_route[route] - start_route[route] >= max_nodes_per_route) {
            index = end_route[route];
            continue;
        }
        //  served_time = time_giant[index] + travel_time[node][giant_tour[index]];
        // adjust_time = served_time + travel_time[giant_tour[index + 1]][node] - travel_time[giant_tour[index + 1]][giant_tour[index]]
        int served_time = time_giant[index] + travel_time[node][giant_tour[index]];
        if (!checkTimeNode(node, served_time)) continue;
        int adjust_time = travel_time[node][giant_tour[index]] + travel_time[giant_tour[index + 1]][node] - travel_time[giant_tour[index + 1]][giant_tour[index]];
        ...
        if (n_negative + getNegativeStudentsWithStartTime(index + 1, n_giant_tour, served_time + travel_time[giant_tour[index + 1]][node]) + getNegative(node, served_time) > max_negative_students) continue;
...
inline void Sol::outFile(double gap)
{
    ...
    //fileOut.open("output/out-" + instance_name);
    fileOut << instance_name << "\t" << n_node << "\t" << res_route << "\t" << res_1 << "\t" << res_2 << "\t" << res_3 << "\t"
            << ((double)n_negative / total_demand) * 100 << "\t" << total_cost << "\t" << runtime << "\t" << gap << endl;
    for (int i = 0; i < n_giant_tour; i++)
        fileOut << giant_tour[i] << " ";
    fileOut.close();
}
