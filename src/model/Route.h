...
void Route::hamilton() {
	...
	// init
	for (int last = 0; last < n_node; last++) {
		f[1 << last][last][0] = travel_time[orderNode[last]][0];
	}
	// calculate
		...
		for (int last = 0; last < n_node; last++)
			for (int negative = 0; negative <= max_negative; negative++) if (f[state][last][negative] != oo) {
				for (int next = 0; next < n_node; next++) if (!(state & (1 << next))) {
					int riding_time = f[state][last][negative] + travel_time[orderNode[next]][orderNode[last]];
					if (!checkTimeNode(orderNode[next], riding_time)) continue;
					int new_state = state | (1 << next);
					int new_negative = negative + getNegative(orderNode[next], riding_time);
	...
	for (int negative = 0; negative <= max_negative; negative++) {
		...
		int pre_last = 0;
		do {
			routes[negative][--n] = orderNode[res_last];
			pre_last = pre[current_state][res_last][current_negative];
			current_negative -= getNegative(orderNode[res_last], f[current_state][res_last][current_negative]);
